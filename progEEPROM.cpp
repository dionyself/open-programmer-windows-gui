...
// dim=size in bytes		addr=0: 1 byte address     =1: 2 byte address
{
	...
	if(saveLog){
		...
		str.Format("ReadI2C(%d,%d)    (0x%X,0x%X)\n",dim,addr,dim,addr);
		WriteLog(str);
	}
	hvreg=0;
	memEE.RemoveAll();
...
//****************** read ********************
	...
	for(i=0,j=1;i<dim;i+=DIMBUF-4){
		...
		str.Format(strings[S_CodeReading2],i*100/(dim),i);	//"Lettura: %d%%, ind. %05X"
	...
	if(k!=dim){
		str.Format(strings[S_ReadEEErr],dim,k);	//"Errore in lettura area EEPROM, richiesti %d byte, letti %d\r\n"
		PrintMessage(str);
		memEE.SetSize(k);
	}
...
{
	int k=0,z=0,i,j;
	int saveLog,max_errori,errori=0;
	CString str,t,s;
	DWORD BytesWritten=0;
	ULONG Result;
	if(MyDeviceDetected==FALSE) return;
	...
	if(saveLog){
		OpenLogFile(strings[S_LogFile]);	//"Registro.txt"
		str.Format("WriteI2C(%d,%d,%d,%f)    (0x%X,0x%X)\n",dim,addr,page,wait,dim,addr);
		WriteLog(str);
	}
	if(dim>memEE.GetSize()){
		i=memEE.GetSize();
		memEE.SetSize(dim);
		for(;i<memEE.GetSize();i++) memEE[i]=0xFF;
	}
	if(dim<1){
...
//****************** write ********************
	...
	for(i=0,j=1;i<dim;i+=page){
		...
		str.Format(strings[S_CodeWriting2],i*100/(dim),i);	//"Scrittura: %d%%, ind. %04X"
		StatusBar.SetWindowText(str);
		j=1;
		if(saveLog){
			str.Format(strings[S_Log7],i,i,k,k);	//"i=%d(0x%X), k=%d(0x%X)\n"
		...
		if((bufferI[1]==I2C_READ||bufferI[1]==I2C_READ2)&&bufferI[2]<0xFA){
			for(z=3;z<bufferI[2]+3&&z<DIMBUF;z++){
				if(memEE[k++]!=bufferI[z]){
					str.Format(strings[S_CodeVError],i+z-3,i+z-3,memEE[k-1],bufferI[z]);	//"Errore in verifica, indirizzo %04X (%d), scritto %02X, letto %02X\r\n"
					PrintMessage(str);
					errori++;
				}
			}
		}
		str.Format(strings[S_CodeV2],i*100/(dim),i);	//"Verifica: %d%%, ind. %04X"
		StatusBar.SetWindowText(str);
		j=1;
		if(saveLog){
...
// options=0: x16 organization     =1: x8 organization
{
	...
	if(saveLog){
		OpenLogFile(strings[S_LogFile]);	//"Registro.txt"
		str.Format("Read93x(%d,%d,%d)    (0x%X,0x%X)\n",dim,na,options,dim,na);
		WriteLog(str);
	}
	hvreg=0;
	memEE.RemoveAll();
...
// wait=write delay
{
	...
	if(saveLog){
		OpenLogFile(strings[S_LogFile]);	//"Registro.txt"
		str.Format("Write93Sx(%d,%d,%d,%f)    (0x%X,0x%X)\n",dim,na,page,wait,dim,na);
		WriteLog(str);
	}
	if(dim>memEE.GetSize()){
		i=memEE.GetSize();
		...
		if(bufferI[1]==uWRX&&bufferI[2]<0xFA){
			for(z=3;z<bufferI[2]/8+3&&z<DIMBUF;z+=2,k+=2){
				...
				if(memEE[k]!=bufferI[z+1]){
					str.Format(strings[S_CodeVError],i+z-3,i+z-3,memEE[k],bufferI[z+1]);	//"Errore in verifica, indirizzo %04X (%d), scritto %02X, letto %02X\r\n"
					PrintMessage(str);
					errori++;
				}
			}
...
// options=0: x16 organization     =1: x8 organization
{
	...
	if(saveLog){
		OpenLogFile(strings[S_LogFile]);	//"Registro.txt"
		str.Format("Write93Cx(%d,%d,%d)    (0x%X,0x%X)\n",dim,na,options,dim,na);
		WriteLog(str);
	}
	if(dim>memEE.GetSize()){
		i=memEE.GetSize();
		...
		if(options==0){		//x16
			...
			if(bufferI[1]==uWRX&&bufferI[2]<0xFA){
				for(z=3;z<bufferI[2]/8+3&&z<DIMBUF;z++,k++){
					if(memEE[k]!=bufferI[z]){
						str.Format(strings[S_CodeVError],i+z-3,i+z-3,memEE[k],bufferI[z]);	//"Errore in verifica, indirizzo %04X (%d), scritto %02X, letto %02X\r\n"
						PrintMessage(str);
						errori++;
					}
				}
...
// page=page size		wait=write delay
{
	...
	if(saveLog){
		OpenLogFile(strings[S_LogFile]);	//"Registro.txt"
		str.Format("Write25xx(%d,%d,%f)    (0x%X)\n",dim,page,wait,dim);
		WriteLog(str);
	}
	if(dim>memEE.GetSize()){
		i=memEE.GetSize();
...
//****************** verify EEPROM ********************
	...
	else str.Format(strings[S_ComplErr],errori);	//"terminata: %d errori\r\n"
	PrintMessage(str);
//****************** exit ********************
	bufferU[j++]=EN_VPP_VCC;
	bufferU[j++]=0x0;
